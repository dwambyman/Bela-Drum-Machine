...
// This method is called in setup() to set the sample rate
...
void Filter::calculateCoefficients(float frequency, float q)
{
	...
	a0_ = (4*q_ + 2*Omega0*T + pow(Omega0,2)*q_*pow(T,2));
	a1_ = (-8*q_ + 2*pow(Omega0,2)*pow(T,2)*q_);
	a2_ = (4*q_ - 2*Omega0*T + pow(Omega0,2)*q_*pow(T,2));
}
	
// Apply coefficients (calculated in Filter class' calculateCoefficients method) to X[n],X[n-1],X[n-2],Y[n-1],Y[n-2] and
// normalise the result by Y[n]'s coefficient a0
float Filter::highPass(float in)
{
	// Difference equation computation
	float out = ((
       	(b0_ * in) + (b1_ * previousX_[0]) + (b2_ * previousX_[1]))
       	- 
       	((a1_ * previousY_[0]) + (a2_ * previousY_[0])))
       	/a0_;
    
    previousX_[1] = previousX_[0];	// x[n-2] = x[n-1]
    previousX_[0] = in;    // x[n-1] = x[n]
    
    previousY_[1] = previousY_[0];    // y[n-2] = y[n-1]
    previousY_[0] = out;    // y[n-1] = y[n]
